...
/*--------------------------------------------------------------------------*/
/* r = a^b mod P (reduce) */
static void _powmodp_r(uint128_t* r, const uint128_t a, const uint128_t b) {
    uint128_t t = {0};  // Initialize result
    *r = {1};           // Initialize r to 1
    uint128_t base = a;
    uint128_t exp = b;

    while (!(exp.high == 0 && exp.low == 0)) { // While exp is not zero
        if (_u128_is_odd(exp)) {
            _mulmodp(r, *r, base);  // r = r * base
        }
        _mulmodp(&t, base, base);  // t = base * base
        base = t;                  // Update base
        _u128_rshift(&exp);        // Halve the exponent
    }
}
